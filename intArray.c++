...
int main() {
    ...
    cout << "Sorted array: ";
    for(int i = 0; i < size; i++) cout << arr[i] << " "; // 1 2 5 7 9
    cout << endl;
    ...
    cout << "Reversed array: ";
    for(int i = 0; i < size; i++) cout << arr[i] << " "; // 9 7 5 2 1
    cout << endl;

    // 10. find
    auto it = find(begin(arr), end(arr), 7);
    if(it != end(arr)) cout << "Element 7 found at position: " << distance(begin(arr), it) << endl; // 4
    ...
    // 12. all_of
    bool allPositive = all_of(begin(arr), end(arr), [](int x){ return x > 0; });
    cout << "All elements positive: " << (allPositive ? "Yes" : "No") << endl; // Yes

    // 13. any_of
    bool anyGreaterThan10 = any_of(begin(arr), end(arr), [](int x){ return x > 10; });
    cout << "Any element greater than 10: " << (anyGreaterThan10 ? "Yes" : "No") << endl; // No

    // 14. none_of
    bool noneNegative = none_of(begin(arr), end(arr), [](int x){ return x < 0; });
    ...
    cout << "Copied array: ";
    for(int i : arrCopy) cout << i << " "; // 5 2 9 1 7 7 9 3
    cout << endl;

    // 16. transform (doubling each element)
    transform(begin(arr), end(arr), arrCopy, [](int x){ return x * 2; });
    cout << "Transformed array (doubled): ";
    for(int i : arrCopy) cout << i << " "; // 10 4 18 2 14 14 18 6
    cout << endl;
    ...
    for(int i = 0; i < size; i++) cout << arr[i] << " "; // 0 0 0 0 0
    cout << endl;
